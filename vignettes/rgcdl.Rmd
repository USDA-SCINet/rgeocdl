...
by specifying a 'grain method'. In this example, we are also interested in both annual 
and daily data available within July-August 2008, so we will indicate the 'any' grain 
method.
...
Each dataset that has a temporal dimension to the data allows for at least one date "grain", such as yearly, monthly, or daily data. The GCDL infers from our request what temporal grain we are requesting. Here, since we specify year and month data, the GCDL infers that we would like monthly data. If that date grain is supported by a dataset, then we use that grain for that dataset. If that date grain is not supported by a dataset, then GCDL uses the grain method to determine what other grains we can accept. For example, here we specify "finer" to indicate that if monthly data is not available, we are also willing to accept daily data. Other options for grain methods include "strict", "skip" (if grain is not available, don't return data), "any", and "coarser". 
